...
def ParseArgs():
    ...
    parser.add_argument('directoryPath', help='$HOME/Desktop/data/slice/original/image/case_00')
    parser.add_argument('savePath', help='$HOME/Desktop/data/...npy')
    parser.add_argument('--times', type=float, default=10**3)
    parser.add_argument('--numbers', type=int, default=210)
    args = parser.parse_args()

    return args

def main(args):
    ctRange = 2048 * args.times + 1
    ...
    for x in range(args.numbers):
        ...
        for i in p.glob('image*'):
            ...
            imgArray = imgArray * args.times

            hist, _ = np.histogram(imgArray, ctRange, [0, ctRange], density=True)
