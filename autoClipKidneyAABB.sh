...
do
	...
	#python3 clipKidneyAABB.py ${data} ${save} --nonBlack
	python3 clipKidneyAABB.py ${data} ${save}

	if [ $? -eq 0 ]; then
		echo "case_00${number} done."
	
	else
		echo "case_00${number}" >> fail/clipKidneyAABB.txt
