...
def ParseArgs():
    ...
    parser.add_argument("trainingdatafile", help="Input Dataset file for training", nargs=2)
    ...
    parser.add_argument("-t","--testfile", help="Input Dataset file for validation", nargs=2)
...
def main(_):
    ...
    trainingdatalist = ReadSliceDataList3ch3ch_1ch(*args.trainingdatafile)
    testdatalist = None
    if args.testfile is not None:
        testdatalist = ReadSliceDataList3ch3ch_1ch(*args.testfile)
    ...
    with tf.device('/device:GPU:{}'.format(args.gpuid)):
        if not args.noaugmentation:
            if testdatalist is not None:
                ...
                historys = model.fit_generator(ImportBatchArray3ch3ch(trainingdatalist, batch_size = args.batchsize, apply_augmentation = True),
                        steps_per_epoch = int(steps_per_epoch), epochs = args.epochs,
                        callbacks=callbacks,
                        validation_data = ImportBatchArray3ch3ch(testdatalist, batch_size = args.batchsize),
            ...
            if testdatalist is not None:
                ...
                historys = model.fit_generator(ImportBatchArray3ch3ch(trainingdatalist, batch_size = args.batchsize, apply_augmentation = False),
                        steps_per_epoch = int(steps_per_epoch), epochs = args.epochs,
                        callbacks=callbacks,
                        validation_data = ImportBatchArray3ch3ch(testdatalist, batch_size = args.batchsize),
                        validation_steps = len(testdatalist),
