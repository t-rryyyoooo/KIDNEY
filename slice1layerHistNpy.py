...
def ParseArgs():
    ...
    parser.add_argument("alpha", help="save textfile path", default=0.0, type=float)
...
def save_npy_256(imageArray, meta, savePath, is_lab=False):
    LF = sitk.GetImageFromArray(imageArray)
    if is_lab:
        LF = Resampling(LF,256,LF.GetSize(), is_label=True)
    else:
    ...
    imgArry = sitk.GetArrayFromImage(LF)
    np.save(savePath, imgArry)
...
def main(args):
    ...
    for i,kidFrag in enumerate(kidIndex):
        ...
        for ckfl in range(len(cutKidFragLabel[i][0,0,:])):
            ...
            OPL = os.path.join(args.savelabelpath,"label{}_{:02d}.npy".format(i,ckfl))
            OPI = os.path.join(args.saveimagepath,"image{}_{:02d}.npy".format(i,ckfl))
            ...
            save_npy_256(roi_label, label,OPL, is_lab=True)
        ...
        for x ,equalizedImageArray in enumerate(equalizedImageArrayList):
            OPI = os.path.join(args.saveimagepath,"image{}_{:02d}.npy".format(i,x))
            #save_image_256(equalizedImageArray, image , OPI)

            #Save as npy
            save_npy_256(equalizedImageArray, image , OPI)
