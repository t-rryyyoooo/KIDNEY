...
def main(args):
    ...
    aHIST = HIST * args.alpha + (1 - args.alpha) / (2048 * args.times)

    #Make CDF
    cdf = aHIST.cumsum()
    cdf_m = np.ma.masked_equal(cdf,0)
    temp = (cdf_m - cdf_m.min())/(cdf_m.max()-cdf_m.min())
    cdf_m = 2048 * args.times * temp
    cdf = np.ma.filled(cdf_m,0).astype(args.type)
    ...
    for x in range(args.number):
        ...
        for i,l in zip(sorted(pI.iterdir()), sorted(pL.iterdir())):
            ...
            ctImgArray = ctImgArray * args.times
            ctImgArray = np.array(ctImgArray, dtype=np.int)
            
            #Equalizing histogram
            ctImgArray = cdf[ctImgArray] 
            ctImgArray = ctImgArray / args.times - 1024
