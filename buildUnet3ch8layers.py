...
def ImportImageTransformed3d(imagefile, labelfile):
    ...
    for x in imagefile:
        ...
        if x is not None and not check and nono:
            ...
            transformed_image = Transforming3d(image, affine, sitk.sitkLinear, minval)
            imagearry = sitk.GetArrayFromImage(transformed_image)
            non = np.zeros_like(imagearry)
            stackedArray = non
        
        if x is None and check:
            imagearry = np.zeros_like(stackedArray[:,:,0])
    ...
    while True:
        ...
        if apply_augmentation:
            for i in range(0, len(indices), batch_size):
               ...
               onehotlabellist = keras.utils.to_categorical(labellist,num_classes=3)
