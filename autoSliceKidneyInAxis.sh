...
readonly SAVE="$HOME/Desktop/data/slice/6ch"
...
do
	...
	saveOriginal="${SAVE}/original/case_00${number}"
	saveTransform="${SAVE}/transform/case_00${number}"

	echo $data
	echo $saveOriginal
	
	python3 sliceKidney.py ${data} ${saveOriginal}

	if [ $? -eq 0 ]; then
		echo "case_00${number} done."
	
	else
		echo "case_00${number}" >> fail/sliceKidneyInAxis.txt
	...
	python3 sliceKidney.py ${data} ${saveTransform} --suffix _transformed
